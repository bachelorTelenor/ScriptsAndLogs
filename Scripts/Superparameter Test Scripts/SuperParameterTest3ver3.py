...
# Bytt ut disse verdiene her for å endre formel
minRsrp = 101
negRsrpMultiplier = 10

minRsrq = 12
rsrqMultiplier = 4
negRsrqMultiplier = 20

minSpeed = 5
maxSpeed = 10
negSpeedMultiplier = 15
...
with open(logPath) as csv_file:
    ...
    for row in csv_reader:
        if int(row['MERAKIrsrp']) < -111 or int(row['MERAKIrsrq']) < -16:
            rsrp = int(row['MERAKIrsrp'])*-1
            rsrq = int(row['MERAKIrsrq'])*-1
            band = int(row['MERAKIband'])
            speed = round(float(row['IPERFmbs']), 2)

            if rsrp < minRsrp : rsrpSum = minRsrp-rsrp
            else : rsrpSum = negRsrpMultiplier*(minRsrp-rsrp)

            if rsrq < minRsrq : rsrqSum = rsrqMultiplier*(minRsrq-rsrq)
            else : rsrqSum = negRsrqMultiplier*(rsrqMultiplier*(minRsrq-rsrq))

            if speed > maxSpeed : speedSum = maxSpeed - minSpeed
            elif speed < minSpeed : speedSum = negSpeedMultiplier*(speed-minSpeed)
            else : speedSum = speed - minSpeed

            Score = rsrpSum + rsrqSum + speedSum + Base
            Score = round(Score, 2)
            d1.append(Score)
            d1Rsrp.append(rsrp)
            d1Rsrq.append(rsrq)
