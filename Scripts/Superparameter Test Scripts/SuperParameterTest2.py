...
with open(logPath) as csv_file:
    ...
    for row in csv_reader:
        if int(row['MERAKIrsrp']) < -111 or int(row['MERAKIrsrq']) < -16:
            ...
            Score = (rsrpWeight * ((rsrp + minRsrp) / maxRsrp) + rsrqWeight * ((rsrq + minRsrq) / maxRsrq) + bandWeight * ((band - minBand) / maxBand) + (rsrpWeight + rsrqWeight)) / (rsrpWeight + rsrqWeight + bandWeight)
            Score = round(Score, 2)
            d1.append(Score)
            d1Rsrp.append(rsrp)
            d1Rsrq.append(rsrq)
