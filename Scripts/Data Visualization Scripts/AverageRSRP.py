...
with open(logPath) as csv_file:
    ...
    for row in csv_reader:
        if int(row['MERAKIband']) == 800:
            d800.append(float(row['MERAKIrsrp']))
        elif int(row['MERAKIband']) == 900:
            d900.append(float(row['MERAKIrsrp']))
        elif int(row['MERAKIband']) == 1800:
            d1800.append(float(row['MERAKIrsrp']))
        elif int(row['MERAKIband']) == 2100:
            d2100.append(float(row['MERAKIrsrp']))
...
plt.title("Average RSRP per band")
plt.xlabel("Frequency bands (MHz)")
plt.ylabel("RSRP")

plt.tight_layout()
# plt.show()
plt.savefig("Graph/AverageRsrpBandMG21.svg")
