...
logPath = "Logs/KombinertLogMobilTZ.csv"

# plt.style.use("fivethirtyeight")

def addlabels(x,y):
    for i in range(len(x)):
        plt.text(i, y[i]+150, y[i], ha='center')
...
with open(logPath) as csv_file:
    ...
    for row in csv_reader:
        if int(row['GMONBANDint']) == 800:
            d800.append(float(row['GMONBANDint']))
        elif int(row['GMONBANDint']) == 900:
            d900.append(float(row['GMONBANDint']))
        elif int(row['GMONBANDint']) == 1800:
            d1800.append(float(row['GMONBANDint']))
        elif int(row['GMONBANDint']) == 2100:
            d2100.append(float(row['GMONBANDint']))
...
plt.tight_layout()
# plt.show()
plt.savefig("Graph/DatapointsPerBandMobil.svg")
