...
with open(logPath) as csv_file:
    ...
    for row in csv_reader:
        if int(row['MERAKIrsrq']) < -16:
            d1.append(float(row['IPERFmbs']))
        elif int(row['MERAKIrsrq']) < -13:
            d2.append(float(row['IPERFmbs']))
        elif int(row['MERAKIrsrq']) < -10:
            d3.append(float(row['IPERFmbs']))
        elif int(row['MERAKIrsrq']) < -7:
...
plt.title("Average Download Speed per Signal Bar based on RSRQ")
