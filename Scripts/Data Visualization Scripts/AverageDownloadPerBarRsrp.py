...
with open(logPath) as csv_file:
    ...
    for row in csv_reader:
        if int(row['MERAKIrsrp']) < -111:
            d1.append(float(row['IPERFmbs']))
        elif int(row['MERAKIrsrp']) < -102:
            d2.append(float(row['IPERFmbs']))
        elif int(row['MERAKIrsrp']) < -92:
            d3.append(float(row['IPERFmbs']))
        elif int(row['MERAKIrsrp']) < -83:
...
plt.title("Average Download Speed per Signal Bar based on RSRP")
...
plt.tight_layout()
plt.show()
# plt.savefig("Graph/AverageDownloadPerSignalBarRSRP.svg")
