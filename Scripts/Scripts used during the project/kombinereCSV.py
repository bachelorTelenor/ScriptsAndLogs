...
merakiCSV = "Logs/MerakiLogKombinert.csv"
iperfCSV = "Logs/iperf3ClientKombinert.csv"
GPSLoggerCSV = "Logs/GPSLoggerKombinert.csv"
gmonproCSV = "Logs/gmonproKombinert.csv"
kobinertCSV = "Logs/kobinertTestBaseline.csv"

# TODO: Legge til kjøreretning i data basert på tidsperioder
# TODO: Gå igjennom opptak for og skriv ned kjøreretninger med timestamp
# TODO: Legge til knapp i meraki log script for å sette kjøreretning
# TODO: Lag Elasticsearch figurer som viser de forskjellige kjøreretningne
# TODO: Set opp Elasticsearch for bare data fra mobil for å få flere punkter
# TODO: Se på å kjøre iperf fra mobil
...
csvHeader = ['MERAKIAPIquery', 'MERAKIAPIupdate', 'MERAKIrsrp', 'MERAKIrsrpPos', 'MERAKIrsrq', 'MERAKIrsrqPos',
             'MERAKIband', 'IPERFtimestamp', 'IPERFmbs', 'GMONPLMN', 'GMONSYSTEM', 'GMONXCI', 'GMONxNBID',
...
def matchIperf(time):
    ...
    with open(iperfCSV, 'r') as iperfData_csv:
        iperfData = csv.reader(iperfData_csv)
        next(iperfData)
        for l in iperfData:
            iperfTime = l[0]
        ...
        for l in gpsData:
            ...
            if (timeDif < datetime.timedelta(seconds=5)):
                if (timeDif <= bestMatch):
                    ...
                    time1 = gpstimedatetime.strftime('%Y-%m-%dT%H:%M:%SZ')
                    lat = l[1]
                    lon = l[2]
                    location = f'{lat},{lon}'
                    elevation = l[3]
...
with open(merakiCSV, 'r') as merakiData_csv:
    ...
    for row in merakiData:
        ...
        merakiCSVrow = [row[0], merakiTimeCSV, row[2], abs(int(row[2])), row[3], abs(int(row[3])), row[4]]
